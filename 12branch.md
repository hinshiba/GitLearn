...
このスラッシュはディレクトリのような階層を作るものではなく，名前を整理するための慣習である．
<!-- feat/hoge があるときに feat ブランチが作成できない -->

<!-- 多くの場合? mainに切り替わったバージョンがあるはずです． またconfigでデフォルトブランチ名が設定できます 00で説明しているはずです -->
Gitで最初から存在するブランチを**デフォルトブランチ**といい，多くの場合その名前は`main`である．
これまでの説明においては`main`ブランチにコミットを追加してきた．
...
移動すると，**HEAD**が指す先がそのブランチに変わり，ワーキングツリーの内容もそのブランチのコミットの状態へ更新される．
...
> git swich -c <ブランチ名> は現在のコミットを指す新しいブランチを作成することに注意せよ
...
<!-- ![ブランチ操作](draw/branch.webp) 画像は別途作成 -->
